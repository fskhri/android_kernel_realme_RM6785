...
#include "ov02a10mipiraw1_Sensor.h"
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    LOG_INF("ov02a10_mipi_raw1 I2C_BUS = %d\n", I2C_BUS);
    if(I2C_BUS != 2){
			...
			*sensor_id = ((read_cmos_sensor(0x0200) << 8) | read_cmos_sensor(0x0300));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (deviceInfo_register_value == 0x00) {
                                        Oplusimgsensor_Registdeviceinfo("Cam_r3", DEVICE_VERSION_OV02A10, imgsensor_info.module_id);
...
UINT32 OV02A10_MIPI_RAW1_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
