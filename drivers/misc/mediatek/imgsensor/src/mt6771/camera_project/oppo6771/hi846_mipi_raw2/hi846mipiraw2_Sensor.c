...
#include "hi846mipiraw2_Sensor.h"
...
static kal_int32 write_eeprom_protect(kal_uint16 enable)
{
	...
	pr_debug("protect 0x8000 val: 0x%x\n", read_cmos_eeprom_8(0x8000));
				...
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
                    ...
                    if (deviceInfo_register_value == 0x00) {
                        Oplusimgsensor_Registdeviceinfo("Cam_r1", DEVICE_VERSION_HI846, imgsensor_info.module_id);
...
UINT32 HI846_MIPI_RAW2_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
