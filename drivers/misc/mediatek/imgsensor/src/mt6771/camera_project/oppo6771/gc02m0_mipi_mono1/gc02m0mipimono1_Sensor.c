...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC02M0_SENSOR_ID1,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("gc02mo_mipi_mono1_Sensor I2C_BUS = %d\n", I2C_BUS);
	if(I2C_BUS != 4){
		*sensor_id = 0xFFFFFFFF;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			if (deviceInfo_register_value == 0x00) {
				Oplusimgsensor_Registdeviceinfo("Cam_r2", DEVICE_VERSION_GC02M0, imgsensor_info.module_id);
...
UINT32 GC02M0_MIPI_MONO1_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
