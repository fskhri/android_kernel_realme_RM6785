...
#include "ov02a10mipiraw2_Sensor.h"
...
static struct imgsensor_struct imgsensor = {
	#ifdef OPLUS_FEATURE_CAMERA_COMMON
	/*weiriqin@Camera.Drv, 20190617, add for set correct mirror/flip */
...
/*static void capture_15fps_setting(kal_uint16 currefps)
{
	LOG_INF("E! currefps:%d\n", currefps);
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x05, 0x06);
	write_cmos_sensor(0x06, 0x14);
	write_cmos_sensor(0xac, 0x00);
}*/
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("ov02a10_mipi_raw2 I2C_BUS = %d\n", I2C_BUS);
	if(I2C_BUS != 4){
			...
			*sensor_id = ((read_cmos_sensor(0x0200) << 8) | read_cmos_sensor(0x0300)) + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (deviceInfo_register_value == 0x00) {
                                        Oplusimgsensor_Registdeviceinfo("Cam_r2", DEVICE_VERSION_OV02A10, imgsensor_info.module_id);
...
static kal_uint32 open(void)
{
	 ...
	 */
	/*weiriqin@Camera.Drv,2019/07/26, add for ov02a10 need to wait more than 9ms and than start i2c communication after avdd dvdd iodvdd mclk rst pin power on, according to datasheet*/
	mdelay(9);
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
...
UINT32 OV02A10_MIPI_RAW2_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
