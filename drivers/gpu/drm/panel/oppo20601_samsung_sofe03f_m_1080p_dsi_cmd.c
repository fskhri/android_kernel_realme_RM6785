		...
		void *handle, unsigned int hbm_mode)
{
	int i = 0;
	int level = 0;
	int oppo_display_brightness_exp = 0;
	if (!cb)
		return -1;

	if ((oppo_display_brightness < BRIGHTNESS_HALF) && (oppo_display_brightness > 0)) {
                oppo_display_brightness_exp = map_exp[oppo_display_brightness];
        } else {
                oppo_display_brightness_exp = oppo_display_brightness;
        }
	pr_err("oppo_display_brightness= %ld, oppo_display_brightness_exp = %d, hbm_mode=%u\n", oppo_display_brightness, oppo_display_brightness_exp, hbm_mode);
	...
	if(hbm_mode == 1) {
		...
		level = oppo_lcm_dc_backlight(dsi,cb,handle, oppo_display_brightness_exp, 0);
