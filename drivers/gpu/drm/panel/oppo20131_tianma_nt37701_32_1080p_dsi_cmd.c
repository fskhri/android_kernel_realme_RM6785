...
#include "../mediatek/mtk_corner_pattern/oppo20131_TM_NT37701_data_hw_roundedpattern.h"
#endif
...
/*Xuhang.Li@BSP.TP, 2020/10/29, Add for tp charge rate before lcm charge refresh*/
...
/*lisheng@PSW.MM.LCD.Display.Stability, 2020/09/01, add for display panel power*/
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	//lcm_dcs_write_seq_static(ctx,0xB5,0x7F,0x46,0x11,0x50);
	//lcm_dcs_write_seq_static(ctx,0x6F,0x0B);
	//lcm_dcs_write_seq_static(ctx,0xB5,0x48,0x00,0x50);
...
static int oppo_lcm_dc_backlight(void *dsi, dcs_write_gce cb,void *handle,unsigned int level,int hbm_en)
{
	...
	if (!oplus_dc_enable_real || hbm_en || level < 4) {
		goto dc_disable;
	}
		...
		void *handle, unsigned int hbm_mode)
{
	...
	pr_err("TM oppo_display_brightness= %ld, hbm_mode=%u\n", oppo_display_brightness, hbm_mode);
	...
	if(hbm_mode == 1) {
		...
		lcm_setbrightness(dsi, cb, handle, oppo_display_brightness); //level
		printk("TM %s : %d ! backlight %d !\n",__func__, hbm_mode, oppo_display_brightness);
	}
			      ...
			      dcs_write_gce cb, void *handle, bool en)
{
	...
	pr_err("TM lcm: debug for oppo_display_brightness= %ld, en=%u\n", oppo_display_brightness, en);
	...
	if(en == 1) {
		...
		if(oplus_dc_alpha){
			for (i = 0; i < sizeof(lcm_finger_HBM_DC_ADD)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, lcm_finger_HBM_DC_ADD[i].para_list, lcm_finger_HBM_DC_ADD[i].count);
			}
		}
...
static int lcm_set_power(unsigned int voltage_id, unsigned int voltage_value)
{
	...
	case 1: {
			ret = fan53870_ldo1_set_voltage(voltage_value);
			if (ret < 0) {
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "oppo20131_tianma_nt37701_32_1080p_dsi_cmd",
		.owner = THIS_MODULE,
